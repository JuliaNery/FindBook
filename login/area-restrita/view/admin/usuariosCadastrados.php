...
<head>
	...
	<link rel="stylesheet" href="css/usuariosCadastrados.css">
...
<body>
	...
	<section id="sidebar">
		...
		<ul class="side-menu">
			...
			<li><a href="usuariosCadastrados.php" class="active"><i class='bx bxs-user icon' ></i> Leitores cadastrados</a></li>
			  ...
			  <div class="container">
                <div class="table-wrapper">
                    <table>
                        ...
                        <tbody>
							<?php if(!isset($_GET['pesquisa'])){foreach ($listaLeitor as $linhas) {  ?>
									<tr>
										<td>
											<div class="user">
											<img class="user-image" src="<?php if(!empty($linhas['fotoLeitor'])){ echo "../../../../".$linhas['fotoLeitor']; }else{ echo("https://jamesrmoro.me/wp-content/uploads/2021/02/profile.png");}?>"" alt="">
											<div class="user-info">
										</td>
										<td><?php echo $linhas['nomeLeitor'] ?></td>
										<td><?php echo $linhas['emailLeitor'] ?></td>
										<td><?php echo $linhas['cpfLeitor'] ?></td>
										<td><?php echo $linhas['rgLeitor'] ?> </td>
										<td><?php echo $linhas['dtNascLeitor'] ?></td>
										<td><?php echo $linhas['generoLeitor'] ?></td>
								...
								<?php }}else{ 
									...
									$stmt = $conexao->prepare("SELECT *, DATE_FORMAT(dtNascLeitor,'%d/%m/%Y') AS nasc FROM tbLeitor
																WHERE 
																nomeLeitor like :busca 
																or cpfLeitor like :busca 
																or rgLeitor like :busca 
																or dtNascLeitor like :busca 
																
																order by idLeitor asc");
